...
import Jubo.JuLiao.IM.Wx.Proto.HistoryMsgPushNotice.HistoryMsgPushNoticeMessage;
...
@Service
public class HistoryMsgPushNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	HistoryMsgPushNoticeMessage req = vo.getContent().unpack(HistoryMsgPushNoticeMessage.class);
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						new Thread(
				    			new Runnable() {
									public void run() {
										MessageUtil.sendJsonMsg(chx, EnumMsgType.HistoryMsgPushNotice,NettyConnectionUtil.getNettyId(chx), null, req);
									}
								}
				    	).start();
					}
