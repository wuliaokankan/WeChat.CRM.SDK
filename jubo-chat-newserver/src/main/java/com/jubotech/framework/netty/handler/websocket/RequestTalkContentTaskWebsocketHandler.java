...
@Service
public class RequestTalkContentTaskWebsocketHandler{
	private  final Logger log = LoggerFactory.getLogger(getClass());
	/**
	 * 获取聊天消息的原始内容（主要是xml内容）---pc端经过服务端转发给手机端
	 * @param ctx
	 * @param vo
	 */
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo, String contentJsonStr) {
        try {
        	...
        	if(null != cx){
        		MessageUtil.sendMsg(cx, EnumMsgType.RequestTalkContentTask, NettyConnectionUtil.getNettyId(ctx), null, req);
        		//告诉对方消息已收到
