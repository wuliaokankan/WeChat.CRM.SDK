...
@Service
public class ChatRoomMembersNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	ChatRoomMembersNoticeMessage req = vo.getContent().unpack(ChatRoomMembersNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.ChatRoomMembersNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
