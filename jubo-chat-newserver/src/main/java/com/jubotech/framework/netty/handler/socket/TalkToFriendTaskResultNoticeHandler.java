...
import Jubo.JuLiao.IM.Wx.Proto.TalkToFriendTaskResultNotice.TalkToFriendTaskResultNoticeMessage;
...
@Service
public class TalkToFriendTaskResultNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	TalkToFriendTaskResultNoticeMessage req = vo.getContent().unpack(TalkToFriendTaskResultNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
					...
					if (null != accInfo) {
						...
						if (null != chx) {
							MessageUtil.sendJsonMsg(chx, EnumMsgType.TalkToFriendTaskResultNotice, NettyConnectionUtil.getNettyId(chx),null, req);
						}
					}
					// 告诉客户端消息已收到
