...
@Service
public class CircleDelNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	CircleDelNoticeMessage req = vo.getContent().unpack(CircleDelNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.CircleDelNotice, NettyConnectionUtil.getNettyId(chx),null, req);
					}
				}
				// 告诉客户端消息已收到
