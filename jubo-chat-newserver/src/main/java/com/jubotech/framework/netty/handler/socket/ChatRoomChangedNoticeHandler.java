...
import Jubo.JuLiao.IM.Wx.Proto.ChatRoomChangedNotice.ChatRoomChangedNoticeMessage;
...
@Service
public class ChatRoomChangedNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	ChatRoomChangedNoticeMessage req = vo.getContent().unpack(ChatRoomChangedNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.ChatRoomChangedNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
