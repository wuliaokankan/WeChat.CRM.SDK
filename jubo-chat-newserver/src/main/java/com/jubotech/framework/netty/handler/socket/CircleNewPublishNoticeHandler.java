...
import Jubo.JuLiao.IM.Wx.Proto.CircleNewPublishNotice.CircleNewPublishNoticeMessage;
...
@Service
public class CircleNewPublishNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	//log.info(JsonFormat.printer().print(vo.getContent()));
        	CircleNewPublishNoticeMessage req = vo.getContent().unpack(CircleNewPublishNoticeMessage.class);
        	log.info(JsonFormat.printer().print(req));
			// 把消息转发给pc端
			// a、根据wechatId找到accountid
			// b、通过accountid找到account
			// c、通过account账号找到通道
			WeChatAccountInfo account = weChatAccountService.findWeChatAccountInfoByWeChatId(req.getCircle().getWeChatId());
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.CircleNewPublishNotice, NettyConnectionUtil.getNettyId(chx),null, req);
					}
				}
				// 告诉客户端消息已收到
