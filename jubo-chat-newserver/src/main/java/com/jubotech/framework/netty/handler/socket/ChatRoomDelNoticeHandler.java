...
import Jubo.JuLiao.IM.Wx.Proto.ChatRoomDelNotice.ChatRoomDelNoticeMessage;
...
@Service
public class ChatRoomDelNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	ChatRoomDelNoticeMessage req = vo.getContent().unpack(ChatRoomDelNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.ChatRoomDelNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
