...
public class PullChatRoomQrCodeTaskResultNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	PullChatRoomQrCodeTaskResultNoticeMessage req = vo.getContent().unpack(PullChatRoomQrCodeTaskResultNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.PullChatRoomQrCodeTaskResultNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
