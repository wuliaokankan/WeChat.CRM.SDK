...
import Jubo.JuLiao.IM.Wx.Proto.ContactLabelInfoNotice.ContactLabelInfoNoticeMessage;
...
@Service
public class ContactLabelInfoNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	ContactLabelInfoNoticeMessage req = vo.getContent().unpack(ContactLabelInfoNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.ContactLabelInfoNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
