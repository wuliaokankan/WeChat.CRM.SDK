...
import Jubo.JuLiao.IM.Wx.Proto.PostMomentsPraiseCountNotice.PostMomentsPraiseCountNoticeMessage;
...
@Service
public class PostMomentsPraiseCountNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	PostMomentsPraiseCountNoticeMessage req = vo.getContent().unpack(PostMomentsPraiseCountNoticeMessage.class);
    		...
    		if(null != account && null != account.getAccountid() && 1 != account.getIslogined()){
				...
				if(null != accInfo){
				    ...
				    if(null != chx){
				    	MessageUtil.sendJsonMsg(chx, EnumMsgType.PostMomentsPraiseCountNotice, NettyConnectionUtil.getNettyId(chx), null, req);
				    }
				}
				//告诉客户端消息已收到
