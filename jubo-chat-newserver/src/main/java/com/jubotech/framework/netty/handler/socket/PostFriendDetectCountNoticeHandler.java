...
import Jubo.JuLiao.IM.Wx.Proto.PostFriendDetectCountNotice.PostFriendDetectCountNoticeMessage;
...
@Service
public class PostFriendDetectCountNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	PostFriendDetectCountNoticeMessage req = vo.getContent().unpack(PostFriendDetectCountNoticeMessage.class);
    		...
    		if(null != account && null != account.getAccountid() && 1 != account.getIslogined()){
				...
				if(null != accInfo){
				    ...
				    if(null != chx){
				    	MessageUtil.sendJsonMsg(chx, EnumMsgType.PostFriendDetectCountNotice, NettyConnectionUtil.getNettyId(chx), null, req);
				    }
				}
				//告诉客户端消息已收到
