...
import Jubo.JuLiao.IM.Wx.Proto.ChatRoomAddNotice.ChatRoomAddNoticeMessage;
...
@Service
public class ChatRoomAddNoticeHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx, TransportMessage vo) {
        try {
        	ChatRoomAddNoticeMessage req = vo.getContent().unpack(ChatRoomAddNoticeMessage.class);
			...
			if (null != account && null != account.getAccountid() && 1 != account.getIslogined()) {
				...
				if (null != accInfo) {
					...
					if (null != chx) {
						MessageUtil.sendJsonMsg(chx, EnumMsgType.ChatRoomAddNotice,NettyConnectionUtil.getNettyId(chx), null, req);
					}
				}
				// 告诉客户端消息已收到
