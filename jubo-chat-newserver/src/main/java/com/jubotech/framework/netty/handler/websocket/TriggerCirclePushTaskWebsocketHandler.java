...
@Service
public class TriggerCirclePushTaskWebsocketHandler{
	private  final Logger log = LoggerFactory.getLogger(getClass());
	/**
	 * 触发手机推送朋友圈列表任务---pc端经过服务端转发给手机端
	 * startTime传秒
	 * @param ctx
	 * @param vo
	 */
    public  void handleMsg(ChannelHandlerContext ctx,TransportMessage vo, String contentJsonStr) {
        try {
        	...
        	//TriggerCirclePushTaskMessage req = vo.getContent().unpack(TriggerCirclePushTaskMessage.class);
        	//将消息转发送给手机客户端
        	ChannelHandlerContext cx=	NettyConnectionUtil.getClientChannelHandlerContextByUserId(req.getWeChatId());
        	if(null != cx){
        		MessageUtil.sendMsg(cx, EnumMsgType.TriggerCirclePushTask, NettyConnectionUtil.getNettyId(ctx), null, req);
        		// 告诉发送方消息已收到
